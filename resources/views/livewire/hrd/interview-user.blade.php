            ...
            <div class="widget-holder col-md-12">
                <div class="widget-bg">
                    <div class="widget-heading widget-heading-border">
                        <h5 class="widget-title" style="display: flex; align-items: center; gap: 0.5rem;">Data Interview User @livewire('partials.badge-jumlah-proses', ['status' => 'interview_user'], key('interview_user'))</h5>
                    </div>
                                        ...
                                        <table class="table table-hover">
                                            ...
                                            <tbody>
                                                ...
                                                <tr class="animate__animated animate__fadeIn">
                                                    ...
                                                    <td>{{ Carbon\Carbon::parse($candidate->date_interview_user)->format('d/m/y H:i') }}</td>
                                                    <td>{{ $candidate->users->name }}</td>
                                                    <td>
                                                        <a href="{{ $candidate->url_interview_user }}" target="_blank">{{ Str::limit($candidate->url_interview_user, 31, '...') }}</a>
                                                    </td>
                                                    <td>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <p><strong>Status:</strong> 
                                <span class="badge badge-{{ $selectedCandidate->status == 'applied' ? 'info' : ($selectedCandidate->status == 'interview_user' ? 'warning' : 'danger') }}">
    ...
    <div wire:ignore.self class="modal fade show d-block" id="approveModal" tabindex="-1" style="background-color: rgba(0,0,0,0.5);">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>Apakah Anda yakin ingin meloloskan candidate ini ke tahap Psikotest?</p>
                    <p><strong>Silakan pilih tanggal psikotest:</strong></p>
                    <input type="datetime-local" class="form-control" wire:model="psikotestDate" min="{{ $minDate }}" style="margin-top: 10px;">
                    @error('psikotestDate') 
                    ...
                    @error('selectedChecker') 
                        <div class="text-danger mt-1">{{ $message }}</div> 
                    @enderror
                    <p class="mt-2"><strong>Silakan isi link gform:</strong></p>
                    <input type="text" class="form-control" wire:model="psikotestUrl">
                    @error('psikotestUrl') 
                        <div class="text-danger mt-1">{{ $message }}</div> 
                    @enderror
                </div>
