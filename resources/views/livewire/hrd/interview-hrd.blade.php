            ...
            <div class="widget-holder col-md-12">
                <div class="widget-bg">
                    <div class="widget-heading widget-heading-border">
                        <h5 class="widget-title" style="display: flex; align-items: center; gap: 0.5rem;">Data Interview HRD @livewire('partials.badge-jumlah-proses', ['status' => 'interview_hrd'], key('interview_hrd'))</h5>
                    </div>
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6">
                            ...
                            <p><strong>Status:</strong> 
                                <span class="badge badge-{{ $selectedCandidate->status == 'applied' ? 'info' : ($selectedCandidate->status == 'interview_hrd' ? 'warning' : 'danger') }}">
    ...
    <div wire:ignore.self class="modal fade show d-block" id="approveModal" tabindex="-1" style="background-color: rgba(0,0,0,0.5);">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>Apakah Anda yakin ingin meloloskan candidate ini ke tahap Interview User?</p>
                    <p><strong>Silakan pilih tanggal interview User:</strong></p>
                    <input type="datetime-local" class="form-control" wire:model="interviewUserDate" min="{{ $minDate }}" style="margin-top: 10px;">
                    @error('interviewUserDate') 
                        <div class="text-danger mt-1">{{ $message }}</div> 
                    @enderror
                    <p class="mt-3"><strong>Pilih User Terkait:</strong></p>
                    <select class="form-control" wire:model="selectedUser" id="selectUser">
                        <option value="">Pilih User</option>
                        @foreach($users as $user)
                            <option value="{{ $user->id }}">{{ $user->nama_lengkap }} ({{ $user->divisi }})</option>
                        @endforeach
                    </select>
                    @error('selectedUser') 
                        <div class="text-danger mt-1">{{ $message }}</div> 
                    ...
                    <input type="text" class="form-control" wire:model="interviewUserUrl">
                    @error('interviewUserUrl') 
                        <div class="text-danger mt-1">{{ $message }}</div> 
                    @enderror
                </div>
                <div class="modal-footer">
